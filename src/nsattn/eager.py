...
create_block_mask_cached = cache(create_block_mask)
...
class Compressor(nn.Module):
    ...
    def forward(self, k: TT):
        ...
        return self.mlp(k + pe).squeeze(-2)
...
def nsa_attention(
    ...
    *,
    ...
    attn_bias = torch.zeros(S, k_cmp.size(-2), dtype=q.dtype, device=q.device)
    attn_bias.masked_fill_(~torch.ones_like(attn_bias, dtype=torch.bool).tril(), float('-inf'))
    ...
    mask = torch.zeros(*indices.shape[:3], k.size(-2)//selected_block_size, dtype=torch.bool)
    mask = mask.scatter_(-1, indices, True).repeat_interleave(selected_block_size,-1)
